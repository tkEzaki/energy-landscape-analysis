function [hout,T,perm] = dendrogram2(Z,varargin)
...
%   Modified for using minus Z value [SysLab]
...
if nargin > 2
    ...
    % add data input [SysLab]
    if strcmpi(varargin{1+offset}, 'data')
        data = varargin{1+offset+1};
        offset = offset + 2;
    end
    
    if length(varargin) > offset
...
if isempty(data)
    ...
    ymin = min(min(Z(:,3)), min(data));
    ymax = max(max(Z(:,3)), max(data));
end
...
if(~horz)
    ...
    % To fix minus value
    if min(Z(:))<0
        lims = [0 numLeaves+1 ymin-margin (ymax+margin)];
    else
...
if flag == 0 && d ~= 1 % row m is a leaf node.
    T(X(m,1)) = c;
    T(X(m,2)) = c;
end
% ---------------------------------------
