...
% Modified by System Laboratory (2017.524)
%      Change the evaluation criteria for searching minimize the maximum
%      energy in the path.
%
function [costs,paths] = dijkstra(AorV,xyCorE,SID,FID,showWaitbar)
    ...
    for k = 1:L
        ...
        while any(~isSettled(FID))
            ...
            for kk = 1:length(nodeIndex)
                ...
                if ~isSettled(J)
                    ...
                    if empty || (jTable(J) <= jTable(I))
                        % (Modified)
                        %iTable(J) = jTable(I) + c;
                        iTable(J) = max(jTable(I), c);
                        if isReverse
                            path{J} = [J path{I}];
                        else
                            path{J} = [path{I} J];
        ...
        if showWaitbar && ~mod(k,ceil(L/10))
            waitbar(k/L,hWait);
        end
    end
    if showWaitbar
